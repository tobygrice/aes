...
fn expand_key(key: &[[u8; 4]]) -> Vec<[[u8; 4]; 4]> {
    // Variable names match FIPS-192, NIST specification: https://doi.org/10.6028/NIST.FIPS.197-upd1
    ...
    let nk = key.len(); // key size (in 4-byte words)
    ...
    for i in nk..nw {
        ...
        // xor bytes:
        for byte in 0..4 {
            w[i][byte] = temp[byte] ^ w[i - nk][byte];
        }
